...
class DailyLLM(EventHandler):
    def __init__(
        ...
        try:
            ...
            while time.time() < self.expiration and not self.participant_left:
                # all handling of incoming transcriptions happens in
    ...
    def send_languages(self):
        self.client.send_app_message({"msg": "translatorbot", "data": {
                                     "in": self.in_language, "out": self.out_language}})
    ...
    def on_app_message(self, message, sender):
        ...
        if message['msg'] == 'request-languages':
            ...
            print(
                f"Participant langs updated; now it's {self.participant_langs}")
    ...
    def on_transcription_message(self, message):
        ...
        if not re.match(r"tb\-.*", message.get('user_name', '')):
            ...
            if self.participant_langs[message['participantId']
                ...
                print(
                    f"not handling user speech for non-translated language: {message}")

        else:
            print(
                f"💼 Got transcription from translator {message['user_name']}, ignoring")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--in_language",
        type=str,
        help="Language. Try 'french', 'spanish', 'japanese', or 'english'")
    parser.add_argument(
        "-o",
        "--out_language",
        type=str,
